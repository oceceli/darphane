...
mostcol = 'MOST_' + str(mostlength) + '_' + str(mostpercent)

class Darphane(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        mostdf = MOST(dataframe, percent=mostpercent, length=mostlength, MAtype=2)
        dataframe[mostcol] = mostdf[mostcol]
        dataframe[emacol] = mostdf[emacol]
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                (qtpylib.crossed_above(dataframe[emacol], dataframe[mostcol])) |
                (
    ...
    """
    Everything from here completely stolen from the work of @werkkrew
...
def MOST(dataframe,  percent=2, length=8, MAtype=1):
    ...
    if MAtype==1:
        ...
        df[mavalue]=fta.ZLEMA(df, period = length)
